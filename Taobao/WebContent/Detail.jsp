...
.d2_1_1 {
	background-image: url(<%=gd.getPicture()%>);
	background-size: cover;
	height: 90%;
}
		...
		<div class="d2_2">
			...
			<form action="CartServlet" class="f1">
				<table class="ta1">
					<tr>
						<td><%=gd.getName()%></td>
						<td colspan="5"><%=gd.getGoodexplain()%></td>
					</tr>
					<tr>
						<td>价格<input type="hidden" name="GoodsIdDetail"
							value="<%=gd.getId()%>" /></td>
						<td class="td">￥<%=gd.getPrice()%></td>
					</tr>
					<tr>
						<td>淘宝价</td>
						<td>￥<%=gd.getLowprice()%></td>
						<td><%=gd.getLowpriceexplain()%></td>
					</tr>
					<tr>
						<td>数量</td>
						<td><input type="text" value="-" onclick="reduce()" id="1"
							class="in"><input type="text" value="1" name="buynum"
							id="2"><input class="in" type="text" value="+" id="3"
							onclick="add(<%=gd.getNum()%>)"></td>
